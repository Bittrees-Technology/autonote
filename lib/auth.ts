...
export const cookieName =
  process.env.NODE_ENV === "production"
    ? "__Host-autonote-session"
    : "autonote-session";
const challengeCookie =
  process.env.NODE_ENV === "production"
    ? "__Host-autonote-challenge"
    : "autonote-challenge";
  ...
  if (body.kind === "ethereum") {
    ...
    payload = new SiweMessage({
      ...
      statement: body.recoveryToken
        ? "Verify your current AutoNote account before combining accounts. No transactions are authorized."
        : body.link
          ? "Link this wallet to your AutoNote identity."
          : "Sign in to AutoNote. This does not authorize transactions.",
      uri: origin,
      version: "1",
    ...
    if (process.env.RESEND_API_KEY && process.env.EMAIL_FROM) {
      ...
      const response = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          ...
          to: [value],
          subject: "Your AutoNote sign-in code",
          text: `Your AutoNote verification code is ${payload}. It expires in 10 minutes. If you did not request this code, ignore this email.`,
        }),
      });
      if (!response.ok) {
