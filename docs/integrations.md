...
Bearer-only `review-status`, `review-prepare` and `review-receipt` routes expose permission state, bounded staging and metadata-only receipt recovery. Separate source-session/Origin/account-bound routes own enable/disable, detail, save and deletion; a bearer cannot approve. Preparation is bounded to 64 KiB at the transport and 64,000 bytes after validation. Other route bodies retain their 16 KiB bound. Review content and credentials never enter query strings or browser storage; links carry only a review ID. The source list is capped at 100 reviews, with pending payloads ordered first; direct review operations still require ownership and fresh authority. The companion provides explicit staging/receipt controls; production activation remains disabled.


A companion review link opens `/connect/ai?review=<review UUID>` and loads that exact review using the signed-in source account and current authority. Invalid or repeated review parameters show an error; unavailable, revoked or unowned reviews disclose no content. The link never approves or saves. Focus loss clears details and confirmation; “Reopen linked review” performs fresh checks. Reopening a saved review shows its historical receipt rather than a second save action. The manual review list remains available. Synthetic production-browser checks cover these paths; real-source pilot acceptance remains open.
